...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    console.log('[PERMISSIONS GUARD] Permissões requeridas:', requiredPermissions);

    if (!requiredPermissions) {
      console.log('[PERMISSIONS GUARD] Nenhuma permissão requerida, liberando acesso');
      return true;
    }
    ...
    console.log('[PERMISSIONS GUARD] Resultado:', hasPermission ? 'PERMITIDO' : 'NEGADO');
  ...
  private async getUserPermissions(targetUserId: string): Promise<string[]> {
    ...
    console.log(`[PERMISSIONS] Buscando permissões para User ID: ${targetUserId}`);
    ...
    if (userModules.length === 0) {
        console.error('[PERMISSIONS ERROR] UserModuleAccess está VAZIO! Usuário não tem acesso a módulos.');
    ...
    for (const modAccess of userModules) {
      ...
      for (const activity of modAccess.module.activities) {
        console.log(`[PERMISSIONS] - Atividade: ${activity.name}, isMandatory: ${activity.isMandatory}`);
        
        const specificAccess = modAccess.activityAccess.find(a => a.activityId === activity.id);
        
        let isActive = activity.isMandatory; 
        if (specificAccess) {
            isActive = specificAccess.isEnabled;
            console.log(`[PERMISSIONS] -- Acesso específico encontrado: isEnabled=${isActive}`);
        }
        ...
        if (isActive) {
          ...
          activity.permissions.forEach(ap => {
              ...
              console.log(`[PERMISSIONS FOUND] Adicionada chave: ${ap.permission.key}`);
          });
        } else {
