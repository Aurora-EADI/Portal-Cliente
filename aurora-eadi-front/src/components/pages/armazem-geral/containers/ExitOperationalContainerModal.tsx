...
import { Loader2, LogOut, Truck, Package, ChevronRight, ChevronLeft, ClipboardList, CheckCircle2, AlertTriangle } from "lucide-react";
import { toast } from "sonner";
import { useCarriers } from "@/hooks/useCarriers";
import { useContainerExit } from "@/hooks/armazem-geral/useContainers";
import type { OperationalContainer } from "@/types/armazem-geral";
...
interface ExitOperationalContainerModalProps {
  isOpen: boolean;
  onClose: () => void;
  containers: OperationalContainer[];
}

export function ExitOperationalContainerModal({
  isOpen,
  onClose,
  containers,
}: ExitOperationalContainerModalProps) {
  ...
  const { mutateAsync: registerExit, isPending } = useContainerExit();

  const exitableContainers = useMemo(() => {
    const list = (containers ?? []).filter((c) => c.status !== "OUT");
    list.sort((a, b) => a.containerNumber.localeCompare(b.containerNumber, "pt-BR"));
    return list;
  }, [containers]);
  ...
  const selectedContainer = useMemo(
    () => exitableContainers.find((c) => c.id === containerId),
    [exitableContainers, containerId]
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || "Erro ao registrar saída do container.");
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden gap-0 border-none shadow-2xl">
        <div className="bg-primary-600 px-8 py-6">
          <DialogTitle className="text-2xl font-bold text-white">Registrar Saída</DialogTitle>
          <p className="text-primary-100 mt-1">
            {step === 1
              ? "Selecione o container que está saindo"
              : step === 2
          ...
          {step === 1 && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="grid gap-2">
                <Label>Selecione o Contêiner <span className="text-red-500">*</span></Label>
                <Select value={containerId} onValueChange={setContainerId}>
                  <SelectTrigger className="h-12 text-left">
                    <SelectValue placeholder="Busque pelo número do contêiner..." />
                  </SelectTrigger>
                  <SelectContent>
                    {exitableContainers.length === 0 ? (
                      <SelectItem value="__NONE__" disabled>
                        Nenhum contêiner elegível nesta lista
                      </SelectItem>
                    ) : (
                      exitableContainers.map((c) => (
                        <SelectItem key={c.id} value={c.id}>
                          {c.containerNumber} {c.containerType ? `• ${c.containerType}` : ""}
                        </SelectItem>
                      ))
                    )}
                  </SelectContent>
              ...
              {selectedContainer && (
                <div className="p-4 bg-primary-50 border border-primary-100 rounded-lg space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-primary-700">{selectedContainer.containerType || "Não informado"}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500 font-medium">Lacre Original:</span>
                    <span className="font-mono font-bold text-primary-700 uppercase tracking-tighter">{selectedContainer.originalSeal || "-"}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500 font-medium">Localização:</span>
                    <span className="font-bold text-primary-700">{selectedContainer.location || "-"}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 text-left">
              <div className="grid gap-2">
                ...
                <Textarea
                  ...
                  placeholder="Ex.: motivo da saída, destino final, observações de vistorias de saída..."
