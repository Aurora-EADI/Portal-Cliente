...
import { useUpdateVehicle } from "@/hooks/armazem-geral/useTransportadoras";
...
interface EditVehicleModalProps {
  ...
  onClose: () => void;
  carrierId: string;
  vehicle: TransportadoraVehicle;
}

export function EditVehicleModal({ isOpen, onClose, carrierId, vehicle }: EditVehicleModalProps) {
  const { mutateAsync: updateVehicle, isPending } = useUpdateVehicle(carrierId);
  const [plate, setPlate] = useState(vehicle.plate);
  const [type, setType] = useState(vehicle.type || "");
  const [errors, setErrors] = useState<Record<string, string>>({});

  useEffect(() => {
    setPlate(vehicle.plate); setType(vehicle.type || ""); setErrors({});
  }, [vehicle]);

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!plate.trim()) { setErrors({ plate: "A placa é obrigatória." }); return; }
    try {
      await updateVehicle({ vehicleId: vehicle.id, data: { plate: plate.trim().toUpperCase(), type: type.trim() || undefined } });
      ...
      <DialogContent className="sm:max-w-[400px] p-0 overflow-hidden border-none shadow-2xl rounded-2xl focus:outline-none">
        <div className="bg-primary-600 px-6 py-5 relative overflow-hidden">
          ...
          <DialogHeader className="relative z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-xl font-bold text-white">Editar Veículo</DialogTitle>
                <DialogDescription className="text-primary-100 text-sm mt-0.5">Atualize os dados do veículo.</DialogDescription>
              </div>
            </div>
          </DialogHeader>
        </div>
        <form onSubmit={handleSave}>
          <div className="px-6 py-6 space-y-5 bg-white">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Tipo do Veículo</label>
              <div className="relative group">
