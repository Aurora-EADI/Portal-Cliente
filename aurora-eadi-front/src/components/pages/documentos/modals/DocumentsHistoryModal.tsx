                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        {/* Expiration warning in the sidebar list */}
                        {(status === DocumentStatus.APPROVED || status === DocumentStatus.PENDING) && (
                          <>
                            {getValidityStatus(group.latestDoc.dateExpiration) === 'EXPIRED' && (
                              <AlertCircle size={14} className={selectedGroupKey === group.key ? 'text-white' : 'text-red-500'} />
                            )}
                            {getValidityStatus(group.latestDoc.dateExpiration) === 'ALERT' && (
                              <AlertCircle size={14} className={selectedGroupKey === group.key ? 'text-white' : 'text-yellow-500'} />
                            )}
                          </>
                        )}
                      </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="col-span-2 text-sm">
                          <span className={getValidityStatus(doc.dateExpiration) === 'EXPIRED' ? 'text-red-600 font-bold' : (getValidityStatus(doc.dateExpiration) === 'ALERT' ? 'text-yellow-600 font-semibold' : 'text-muted-foreground')}>
                            {formatDateBR(doc.dateExpiration)}
                          </span>
                          {getValidityStatus(doc.dateExpiration) === 'EXPIRED' && (
                            <span className="ml-1 text-[10px] text-red-600 font-bold uppercase">(Vencido)</span>
                          )}
                          {getValidityStatus(doc.dateExpiration) === 'ALERT' && (
                            <span className="ml-1 text-[10px] text-yellow-600 font-bold uppercase">(Alerta)</span>
                          )}
                        </span>
