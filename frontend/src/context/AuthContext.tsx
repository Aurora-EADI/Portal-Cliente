...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUserPermissions = async (userId: string) => {
    if (MOCK_MODE) {
      const mockPermissions: UserPermissionsResult = {
        userId, userName: 'Mock User', totalPermissions: 0, permissions: [],
      };
      setUserPermissions(mockPermissions);
      localStorage.setItem(PERMISSIONS_KEY, JSON.stringify(mockPermissions));
      return;
    }
    try {
